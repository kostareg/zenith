...
| sla rd, rs1, rs2      | arithmetic left shift  |
| sra rd, rs1, rs2      | arithmetic right shift |

[^1]: traps on divide by zero.

Arithmetic overflow is undefined behaviour.
Shift instructions use the low 6 bits of `rs2` as the shift amount. Arithmetic left shift is equivalent to logical
left shift for 64-bit two's-complement values.
...
Unary register instructions use `field1` as `rd`, `field2` as `rs1`, and reserve `field3` and unused bits as zero.
...
When a Zelf executable is loaded, the kernel copies the constant data and code sections into memory, and starts the
child at `code_base + entry_off`. The exact load addresses are chosen by the kernel.
