...
export function ZenithEmulatorProvider({ children }: ZenithEmulatorProviderProps) {
    ...
    return <ZenithEmulatorContext.Provider value={emulator}>{children}</ZenithEmulatorContext.Provider>;
}

export function useZenithEmulator() {
    const context = React.useContext(ZenithEmulatorContext);
    if (context === undefined) {
        throw new Error("useZenithEmulator must be used within a ZenithEmulatorProvider.");
    }
